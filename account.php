...
<title>Project Worlds || Online QUiz System</title>
...
/* Get quiz time from admin */
$time_query = mysqli_query($con,"SELECT time FROM quiz WHERE eid='$eid'") or die('Error');
$time_row = mysqli_fetch_array($time_query);
$quiz_time = $time_row['time'];
if(!isset($_SESSION['start_time_'.$eid])) {
    $_SESSION['start_time_'.$eid] = time();
}

$elapsed_time = time() - $_SESSION['start_time_'.$eid];
$remaining_time = ($quiz_time * 60) - $elapsed_time;
...
echo '
<div class="panel" style="margin:5%; padding:20px; position:relative;">
...
<script>
var totalSeconds = '.$remaining_time.';
...
function startTimer() {
    ...
    clearInterval(timer);
    alert("Time is up!");
    window.location.href="update.php?timeup=true&eid='.$eid.'";
}
        totalSeconds--;
...
while($row=mysqli_fetch_array($q))
{
    ...
    <label style="font-size:18px;">
        <input type="radio" name="ans" value="'.$optionid.'" required>
        '.$option.'
    </label>
    <br /><br />';
}
...
{    $eid=@$_GET['eid'];
unset($_SESSION['start_time_'.$eid]);
